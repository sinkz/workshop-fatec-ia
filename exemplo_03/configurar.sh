...
# ========================================
# 🔧 SCRIPT DE CONFIGURAÇÃO - EXEMPLO 03
# ========================================
...
# Backend
install_deps "backend" "Backend (Proxy Neon MCP)" || FAILED=1
echo ""

# Frontend
install_deps "frontend" "Frontend (Next.js)" || FAILED=1
echo ""
...
if [ ! -f "backend/.env" ] && [ -f "backend/env.example" ]; then
    ...
    echo -e "${YELLOW}   Copie backend/env.example para backend/.env e configure suas credenciais Neon${NC}"
    echo -e "${BLUE}   Veja: docs/NEON_MCP_SETUP.md${NC}"
fi

if [ ! -f "frontend/.env" ] && [ -f "frontend/env.example" ]; then
    echo -e "${YELLOW}⚠️  Arquivo .env não encontrado no frontend${NC}"
    echo -e "${YELLOW}   Copie frontend/env.example para frontend/.env e configure${NC}"
fi
...
if [ $FAILED -eq 0 ]; then
    ...
    echo -e "${GREEN}╚════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "${BLUE}⚠️  IMPORTANTE - Antes de rodar:${NC}"
    echo -e "  1. Configure backend/.env com suas credenciais Neon"
    echo -e "  2. Configure frontend/.env com sua chave Groq"
    echo -e "  3. Veja: docs/NEON_MCP_SETUP.md"
    echo ""
